...
public class SocketPacket {
    ...
    /**
     * 
     * 根据参数构造 类{@link SocketPacket} 对象
     * 
     * @param type
     *            类似 {@link MsgSign#TYPE_RDS_SERVER} hcs端类型值
     * @param flags
     *            通信消息类型值
     * @param data
     *            通信消息json内容
     */
    public SocketPacket(int type, int flags, byte[] data) {
	this.type = (byte) type;
	...
	if (logger.isDebugEnabled()) {
	    logger.debug(Markers.SSLCLIENT, "writeBuffer size:" + buffer.position() + " data:"
		    + (data != null ? new String(data, Middleware.SYSTEM_CHARSET) : "is empty"));
	}
	return buffer;
    }
    ...
    /**
     * @param data
     *            根据 data 设置 {@link #data}{@link #length}的值
     */
     ...
     * @return
     */
    public static final ByteBuffer getWriteByteBuffer(byte version, int flags, byte type, byte[] data) {
    ...
    /**
     * 根据 <a href=
     * 'http://wiki.hotpu.cn:8090/pages/viewpage.action?pageId=13533402'>1.1.1 RS to
     * M_启动成功消息</a> 协议<br>
     * 发送启动成功消息
     * 
     * @param sslClient
    ...
    /**
     * 根据 <a href=
     * 'http://wiki.hotpu.cn:8090/pages/viewpage.action?pageId=14123180'>1.3.1 RS to
     * M_自动异常切换</a> 协议<br>
     * 发送启动成功消息
     * 
     * @param sslClient
     ...
     * @throws IOException
     */
    public static final void sendAutoSwitch(SSLClient sslClient, String errorMsg) throws IOException {
	if (errorMsg != null) {
	    ...
	    ByteBuffer sendBuffer = getWriteByteBuffer(MsgSign.getCurrentVersion(), MsgSign.FLAG_RDS_SERVER_AUTO_SWITCH,
		    MsgSign.TYPE_RDS_SERVER, taskResponse.toString().getBytes(Middleware.SYSTEM_CHARSET));
	...
	ByteBuffer sendBuffer = getWriteByteBuffer(MsgSign.getCurrentVersion(), flag, MsgSign.TYPE_RDS_SERVER,
    ...
    /**
     * 根据 <a href=
     * 'http://wiki.hotpu.cn:8090/pages/viewpage.action?pageId=13533421'>1.2.127 RS
     * to M_心跳</a> 协议 生成心跳buffer<br>
     * 
     * @return 心跳协议buffer
     * 
     */
    public static final ByteBuffer getHeartbeatToMBuffer() {
	ByteBuffer sendBuffer = getWriteByteBuffer(MsgSign.getCurrentVersion(), 0, MsgSign.TYPE_RDS_SERVER,
		MsgSign.getSendDataFromSererIdBytes());
	return sendBuffer;
    }
	...
	debugValue.append("version:").append(version).append(' ').append("flags:").append(flags).append(' ')
		.append("type:").append(type).append(' ').append("length:").append(length);
	if (data != null) {
