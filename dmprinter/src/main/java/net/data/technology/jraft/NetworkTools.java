 ...
 * /HotDB-Cloud-Management/src/main/java/cn/hotdb/cloud/util/SSHOperater.java 秘钥文件 id_rsa id_rsa.pub
 * 要么各账号不一致 要么秘钥文件一个
 */
public class NetworkTools {
     ...
     * 
     * @param ip 目前仅支持ipv4 字符串
     * @param deviceName IP绑定的网卡名
     * @return true 绑定成功 false 绑定失败
     */
    public static boolean addIpByCommand(String ip, String deviceName) {
        ProcessBuilder builder =
                new ProcessBuilder("ip", "addr", "add", ip + "/32", "dev", deviceName);
        ...
        try {
            ...
            if (processComplete != 0) {
                ...
                try {
                    ...
                    if (err != null) {
                        ...
                        if (length >= 0) {
                            ...
                            if (errorInfo.toLowerCase().indexOf("file exists") > 0) {
                                ...
                                // logger.info("run 'ip addr add " + ip + "/32
                                // dev "
                                // + deviceName + "' is error:" + errorInfo);
                                // }
                                System.out.println("run 'ip addr add " + ip + "/32 dev "
                                        + deviceName + "' is error:" + errorInfo);
                                return true;
                            } else {
                                // logger.warn("run 'ip addr add " + ip + "/32
                                // dev"
                                // + deviceName + "' is error:" + errorInfo);
                                System.err.println("run 'ip addr add " + ip + "/32 dev" + deviceName
                                        + "' is error:" + errorInfo);
                            }
                        }
                    }
                    ...
                    // deviceName + "' is error:" + e.getMessage(),e);
                    System.err.println("run 'ip addr add " + ip + "/32 dev" + deviceName
            ...
            System.err.println("run 'ip addr add " + ip + "/32 dev" + deviceName + "' is error:"
                    + e.getMessage());
            e.printStackTrace();
        } finally {
            if (process != null) {
    ...
    public static boolean delIpByCommand(String ip, String deviceName) {
        ProcessBuilder builder =
                new ProcessBuilder("ip", "addr", "del", ip + "/32", "dev", deviceName);
        Process process = null;
        try {
            process = builder.start();
            int processComplete = process.waitFor();
                ...
                try {
                    ...
                    if (err != null) {
                        ...
                        if (length >= 0) {
                            ...
                            if (errorInfo.toLowerCase().indexOf("file exists") > 0) {
                                // if (logger.isInfoEnabled()) {
                                // logger.info("run 'ip addr del " + ip + "/32
                                // dev"
                                // +
                                // deviceName + "' is error:" + errorInfo);
                                // }
                                System.out.println("run 'ip addr del " + ip + "/32 dev" + deviceName
                                        + "' is error:" + errorInfo);
                                return true;
                            } else {
                                // logger.warn("run 'ip addr del " + ip + "/32
                                // dev"
                                // +
                                // deviceName + "' is error:" + errorInfo);
                                System.err.println("run 'ip addr del " + ip + "/32 dev" + deviceName
                                        + "' is error:" + errorInfo);
                            }
                        }
                    }
            ...
            // "' is error:" + e.getMessage(), e);
            System.err.println("run 'ip addr del " + ip + "/32 dev" + deviceName + "' is error:"
                    + e.getMessage());
            e.printStackTrace();
        } finally {
            if (process != null) {
    ...
    public static String getSameNetworkSegmentDeviceByIp(String ip) {
        ...
        try {
            ...
            if (processComplete != 0) {
                ...
                try {
                    ...
                    if (err != null) {
                        ...
                        if (length >= 0) {
                            byte[] errArray = new byte[length];
                            err.read(errArray, 0, length);
                            // logger.warn("run 'ip addr' is error:" + new
                            // String(errArray));
                            System.err.println("run 'ip addr' is error:" + new String(errArray));
                        }
                    }
                } catch (Exception e) {
                    // logger.warn("run 'ip addr' is error:" + e.getMessage()
                    // ,e);
                ...
                String line = null;
                Pattern devPattern = Pattern.compile("(\\d{1})\\:(\\s+)(\\S+)\\:(.*)");// 3
                // dev
                Pattern ipPattern = Pattern.compile("(\\s+)inet(\\s{1})(\\S+)/(.*)"); // 3
                // ip
                Matcher matcher = null;
                HashMap<String, ArrayList<String>> devIpMap =
                ...
                try {
                    ...
                    // logger.warn("run 'ip addr' is error:" +
                    // e.getMessage() ,e);
                    System.err.println("run 'ip addr' is error:" + e.getMessage());
                } finally {
                    if (in != null) {
        ...
        try {
            ...
            if (resInfo != null) {
                if (resInfo.isSuccess()) {
                    ...
                    // logger.warn("remote run 'ip addr' is error:" +
                    // resInfo.getErrRes());
     ...
     * @return true 解除绑定成功 false 解除绑定失败
     */
    public static boolean delIpByRemoteCommand(SSHOperater sshOperater, String ip,
            String deviceName) {
        try {
            ResInfo resInfo =
                    sshOperater.executeSudoCmd("ip addr del " + ip + "/32, dev " + deviceName, 0);
            if (resInfo != null) {
                if (resInfo.isSuccess()) {
                    return true;
                } else {
    ...
    public static void main(String[] args) {
        System.out.println("localIp:" + args[0] + ",vip:" + args[1] + " add result:"
                + addIp(args[0], args[1]));
        try {
            Thread.sleep(20000);
        ...
        System.out.println(
                "localIp:" + args[0] + ",vip:" + args[1] + " del result:" + delIp(args[1]));
    }
