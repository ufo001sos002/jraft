...
CODE >0 则code对应的单个具体错误码，message为错误信息
 CODE = -1 则message为JSON数组信息(即"message":[{code,message}] 其中code = 0 表示成功，>0表示对应错误码，message表示对应 实例ID字符串)
**/
2271011：RDS实例回收 失败(实例信息不存在，一个或多个集群节点)，正常情况下不会出现，除非有BUG
...
2291012：RDS连接用户帐号变更 失败(实例信息不存在，一个或多个集群节点)，正常情况下不会出现，除非有BUG
...
2301013：RDS IP白名单变更 失败(实例信息不存在，一个或多个集群节点)，正常情况下不会出现，除非有BUG
...
2311012：RDS SQL拦截 失败(实例信息不存在，一个或多个集群节点)，正常情况下不会出现，除非有BUG
...
2321011：RDS实例数据库删除 失败(实例信息不存在，一个或多个集群节点)，正常情况下不会出现，除非有BUG
