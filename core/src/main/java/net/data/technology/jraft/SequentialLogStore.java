...
public interface SequentialLogStore {
     ...
     * however, after some compact actions, this could be anything greater or equals
     * to one<br/>
     * (数据记录存储的开始索引)
     * 
     ...
     * (最后存储的数据记录)
     * 
     * @return a dummy constant entry with value set to null and term set to zero if
     *         no log entry in store<br/>
     *         (非空)
     */
     ...
     * 
     * @param index
     *            a value &lt; {@code this.getFirstAvailableIndex()}, and starts
     *            from 1
     * @param logEntry
     *            log entry to write
     */
     ...
     * (根据起始位置返回数据记录集合,注:读不到将抛错)
     * 
     * @param start
     *            the start index of log entries
     * @param end
     *            the end index of log entries (exclusive)
     ...
     *         {@link #getFirstAvailableIndex()}<br/>
     *         (可能为null)
     */
     ...
     * 
     * @param index
     *            index of the log store to start
     * @param itemsToPack
     *            items to pack
     * @return log pack
     */
     ...
     * 
     * @param index
     *            the log index that start applying the logPack, index starts from 1
     * @param logPack
     *            log pack to apply
     */
    ...
    /**
     * Compact the log store by removing all log entries including the log at the
     * lastLogIndex<br/>
     * (通过删除所有 最后 数据记录索引处 及之前的 数据记录 来压缩 数据记录存储)
     * 
     * @param lastLogIndex
     *            pack until the last log index
